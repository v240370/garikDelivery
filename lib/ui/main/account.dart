...
import 'package:delivery_template/model/server/changePassword.dart';
...
class _AccountScreenState extends State<AccountScreen> {
  ...
  _successChangeProfile(){
    openDialog(strings.get(113)); // "User Profile change",
  ...
  _callbackChange(){
    ...
    if (editControllerNewPassword1.text != editControllerNewPassword2.text)
      return openDialog(strings.get(108)); // "Passwords don't equals",
    if (editControllerNewPassword1.text.isEmpty || editControllerNewPassword2.text.isEmpty)
      return openDialog(strings.get(109)); // "Enter New Password",
  ...
  _successChangePassword(){
    openDialog(strings.get(110)); // "Password change",
    pref.set(Pref.userPassword, editControllerNewPassword1.text);
  }

  _errorChangePassword(String error){
    if (error == "1")
      return openDialog(strings.get(111)); // Old password is incorrect
    if (error == "2")
      return openDialog(strings.get(112)); // The password length must be more than 5 chars
